...
### Mixture-of-Agents 综合器（v17.0 P2 · 对应 v17 §2.6）

> 来源：Mixture-of-Agents（2024）+ 2026 多模型协作综述。模型路由两范式——**选择**（选一个最合适的，v14.0 现状）与**综合**（多模型并行生成 + 门控综合，v17.0 新增）。MoA 用多 LLM 协作超越单模型，降低单模型幻觉/偏见。

**何时用 MoA（综合）vs 单模型（选择）**：

| 任务类型 | 模式 | 理由 |
|---------|------|------|
| 简单查询/路由/分类 | 选择（Fast） | 成本敏感，单模型够用 |
| 常规分析/写作 | 选择（Smart） | 性价比均衡 |
| **复杂决策**（赢单策略/价值量化/客户洞察深度分析） | **综合（MoA）** | 单模型幻觉风险高，多模型交叉验证 |
| **关键交付物**（方案/报价/POC 策略） | **综合（MoA）+ 辩论审计** | 高风险，必须交叉审核 |
...
复杂决策任务到达
   ↓
[1. 并行生成] 2-3 个不同模型独立生成候选方案
   模型A（Frontier · 强推理）→ 候选A
   模型B（Smart · 不同厂商）→ 候选B   ← 不同厂商降低同源偏见
   模型C（深度思考引擎/苏格拉底 · 批判视角）→ 候选C
   ↓
[2. 门控综合] 门控网络（可由 Frontier 担任）聚合：
   - 加权各候选（权重来自评测集历史表现：哪个模型在"商机评分"上更准）
   - 识别分歧点 → 触发辩论（呼应 v14.0 Multi-Agent Debate）
   - 综合输出：取共识 + 标注分歧
   ↓
[3. 质量门] 评测集运行器对综合输出评分
   ├── ≥8.5 → 交付
   └── <8.5 → 回到[1]加一轮或降级为单模型最优
   ↓
输出：综合方案 + 推理链 + 分歧标注
```

**门控权重来源**（与可观测层 Cost/Eval 反馈咬合）：

```json
{
  "moa_weights_source": "evaluation_runner 历史表现",
  "weights": {
    "model_A_frontier": 0.45,
    "model_B_smart": 0.30,
    "model_C_critic": 0.25
  },
  "weight_update_trigger": "eval_regression / periodic_review",
  "cost_guardrail": "MoA 总 Token ≤ 单模型 × 3.5（超限降级为单模型最优）"
}
```

**与已建组件咬合**：
- **自进化事件总线**（§2.8）：`eval_regression` 事件 → 自动调整 MoA 权重（哪个模型回归了就降权）
- **可观测层 Cost**：MoA 是重负载，靠 Cost 反馈决定何时降级为单模型（成本守门）
- **深度思考引擎/苏格拉底拷问引擎**：作为 MoA 的"批判模型"候选C（呼应 v14.0 Multi-Agent Debate）
- **上下文工程 Caching**：多模型并行的共同上下文（方法论规则）走缓存复用，控成本
- **程序记忆**：MoA 权重配置作为 `routing/moa_weights.yaml` 可被自进化改写

> **设计哲学（道）**：选择是"赌一个模型对"，综合是"让多个模型互相纠错"。销售的关键决策（赢单策略、价值量化）错一次代价远大于多花 3 倍 Token。MoA 把"单点失误风险"换成"多模型共识成本"——在关键决策上，这是值得的交易。
